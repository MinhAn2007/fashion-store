...
const EditProfile = () => {
    const [userInfo, setUserInfo] = useState({
        ...
        lastName: "",
        email: "",
        addresses: [], // Khởi tạo addresses là một mảng rỗng
    });
    ...
    useEffect(() => {
        const fetchUserProfile = async () => {
            try {
                ...
                setUserInfo({ ...data.user, addresses: data.user.addresses || [] }); // Đảm bảo addresses là một mảng
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            navigate("/account"); // Chuyển hướng về trang thông tin sau khi cập nhật thành công
        } catch (error) {
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {userInfo.addresses.map((address, index) => (
                        <div key={index} className="p-4 mt-4 bg-gray-100 rounded-lg shadow-md border border-gray-200">
                            ...
                            <input
                                type="text"
                                value={address.addressLine || ""} // Đảm bảo có giá trị mặc định
                                onChange={(e) =>
                            ...
                            <input
                                type="text"
                                value={address.city || ""} // Đảm bảo có giá trị mặc định
                                onChange={(e) =>
                    ...
                    <button type="submit" className="mt-6 w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-black hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black">
                        Lưu thay đổi
                    </button>
                </form>
            </div>
